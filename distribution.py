...
class LaplaceDistribution:
    ...
    def mean_abs_deviation_from_median(x: np.ndarray):
        ...
        return median, np.mean(abs(x - median), axis=0) / x.shape[0]
        ####
    ...
    def __init__(self, features):
        ...
        self.scale = np.sum(abs(features - self.loc), axis=0) / features.shape[0]  # YOUR CODE HERE
        ####
    ...
    def pdf(self, values):
        ...
        Args:
            ...
            Every column represents all available values for the selected feature.
        '''
        return np.exp(self.logpdf(values)) / (2 * self.scale)
