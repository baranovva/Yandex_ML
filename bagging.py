...
class SimplifiedBaggingRegressor:
    ...
    def predict(self, data):
        ...
        l = []
        for bag in range(self.num_bags):
            l.append(self.models_list[bag].predict(data))
        l = np.asarray(l).T
        return np.mean(l, axis=1)
    ...
    def _get_oob_predictions_from_every_model(self):
        ...
        for i, obj in enumerate(self.data):
            for model, inxs in zip(self.models_list, self.indices_list):
                if i not in inxs:
    ...
    def _get_averaged_oob_predictions(self):
        ...
        self.oob_predictions = np.array(
                [np.mean(pred) if len(pred) > 0 else None for pred in self.list_of_predictions_lists])
    ...
    def OOB_score(self):
        ...
        preds_inxs = [i for i, pred in enumerate(self.oob_predictions) if pred is not None]
        y_true = self.target[preds_inxs]
        predictions = self.oob_predictions[preds_inxs]
